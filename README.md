...
собирает строки из **двух** источников — литералы `t(...)` в `src/` и каталог
бэкенда (`title`/`description`/`recommendation`/`category` каждого правила в
`rules.rs` и `secrets.rs`) — и падает, если какой-то строки нет в словаре. Так
новое правило не уедет в релиз без перевода. Строки без кириллицы (`Path
traversal`, `document.write()`) пропускаются: откат к исходнику уже даёт
