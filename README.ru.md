...
**Анализ выполняется на вашем компьютере.** Наружу уходит ровно два запроса, и ни
в одном нет ни строчки вашего кода:

- **[OSV.dev](https://osv.dev)** — список имён и версий пакетов, и только при
  включённой проверке CVE.
- **Лента релизов** — номер версии, один раз за запуск, чтобы сказать, что вышла
  новее. Ничего не скачивает и не запускает — ссылку вы открываете сами.
  Отключается в `Ctrl+,` → «Сеть».

Офлайн-режим убирает оба. Телеметрия и проверка найденных секретов на стороне
провайдера принудительно выключены и включиться не могут.
...
| Файл | Что это |
|---|---|
| `VulnScope_1.1.0_x64-setup.exe` | Установщик NSIS — обычный выбор |
| `VulnScope_1.1.0_x64_en-US.msi` | MSI-пакет, для развёртывания через групповые политики |
