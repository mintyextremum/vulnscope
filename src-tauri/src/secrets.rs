...
pub static SECRET_RULES: &[SecretRule] = &[
    ...
    SecretRule {
        id: "VS-SEC-032",
        title: "Токен Atlassian (Jira/Confluence)",
        description: "API-токен Atlassian даёт доступ к Jira и Confluence вашего аккаунта: задачам, страницам и вложениям.",
        recommendation: "Отзовите токен в настройках Atlassian (Security → API tokens) и выпустите новый.",
        severity: Severity::High,
        confidence: Confidence::High,
        pattern: r"\b(ATATT3[A-Za-z0-9_=\-]{20,})",
        value_group: 1,
        min_entropy: 0.0,
        cwe: &["CWE-798"],
    },
    SecretRule {
        id: "VS-SEC-033",
        title: "Linear API-ключ",
        description: "Ключ Linear API даёт доступ к задачам, проектам и комментариям вашей рабочей области.",
        recommendation: "Отзовите ключ в настройках Linear (API) и выпустите новый.",
        severity: Severity::High,
        confidence: Confidence::High,
        pattern: r"\b(lin_api_[A-Za-z0-9]{40})\b",
        value_group: 1,
        min_entropy: 0.0,
        cwe: &["CWE-798"],
    },
    SecretRule {
        id: "VS-SEC-034",
        title: "Токен Doppler",
        description: "Токен Doppler открывает доступ к секретам проектов и окружений, которыми управляет Doppler. Это ключ ко всем остальным секретам.",
        recommendation: "Отзовите токен в панели Doppler и выпустите новый с минимальной областью.",
        severity: Severity::Critical,
        confidence: Confidence::High,
        pattern: r"\b(dp\.(?:pt|st|ct|sa|scim|audit)\.[A-Za-z0-9]{40,44})\b",
        value_group: 1,
        min_entropy: 0.0,
        cwe: &["CWE-798"],
    },
    SecretRule {
        id: "VS-SEC-035",
        title: "Токен PlanetScale",
        description: "Токен PlanetScale даёт доступ к базам данных, веткам и паролям подключения вашей организации.",
        recommendation: "Отзовите токен в настройках PlanetScale и выпустите новый.",
        severity: Severity::Critical,
        confidence: Confidence::High,
        pattern: r"\b(pscale_(?:tkn|pw|oauth)_[A-Za-z0-9_\.\-]{32,})\b",
        value_group: 1,
        min_entropy: 0.0,
        cwe: &["CWE-798"],
    },
    SecretRule {
        id: "VS-SEC-036",
        title: "Токен HashiCorp Vault",
        description: "Токен Vault даёт доступ к секретам, которыми управляет хранилище: ключам, паролям и сертификатам в рамках своих политик.",
        recommendation: "Отзовите токен (vault token revoke) и выпустите новый с ограниченным TTL и политиками.",
        severity: Severity::Critical,
        confidence: Confidence::High,
        pattern: r"\b(hvs\.[A-Za-z0-9_-]{24,})",
        value_group: 1,
        min_entropy: 0.0,
        cwe: &["CWE-798"],
    },
    SecretRule {
        id: "VS-SEC-037",
        title: "Токен сервис-аккаунта Grafana",
        description: "Токен Grafana даёт доступ к дашбордам, источникам данных и настройкам мониторинга.",
        recommendation: "Отзовите токен в Grafana (Service accounts) и выпустите новый.",
        severity: Severity::High,
        confidence: Confidence::High,
        pattern: r"\b(glsa_[A-Za-z0-9]{32}_[A-Fa-f0-9]{8})\b",
        value_group: 1,
        min_entropy: 0.0,
        cwe: &["CWE-798"],
    },
    SecretRule {
        id: "VS-SEC-038",
        title: "Токен Dropbox",
        description: "Токен Dropbox даёт доступ к файлам и папкам аккаунта: чтение, запись и удаление.",
        recommendation: "Отзовите токен в App Console Dropbox и выпустите новый с нужной областью.",
        severity: Severity::High,
        confidence: Confidence::High,
        pattern: r"\b(sl\.[A-Za-z0-9_-]{130,})",
        value_group: 1,
        min_entropy: 0.0,
        cwe: &["CWE-798"],
    },
    SecretRule {
        id: "VS-SEC-039",
        title: "Токен Terraform Cloud",
        description: "Токен Terraform Cloud даёт доступ к рабочим областям, состоянию (state) и переменным — включая другие секреты инфраструктуры.",
        recommendation: "Отзовите токен в настройках Terraform Cloud (Tokens) и выпустите новый.",
        severity: Severity::Critical,
        confidence: Confidence::High,
        pattern: r"\b([a-z0-9]{14}\.atlasv1\.[A-Za-z0-9_-]{60,})",
        value_group: 1,
        min_entropy: 0.0,
        cwe: &["CWE-798"],
    },
    SecretRule {
        id: "VS-SEC-040",
        title: "Серверный ключ Firebase Cloud Messaging",
        description: "Серверный ключ FCM позволяет рассылать push-уведомления всем пользователям вашего приложения.",
        recommendation: "Отзовите ключ в консоли Firebase (Cloud Messaging) и перейдите на HTTP v1 API с OAuth.",
        severity: Severity::High,
        confidence: Confidence::High,
        pattern: r"\b(AAAA[A-Za-z0-9_-]{7}:APA91[A-Za-z0-9_-]{130,})",
        value_group: 1,
        min_entropy: 0.0,
        cwe: &["CWE-798"],
    },
    SecretRule {
        id: "VS-SEC-041",
        title: "Токен Adobe",
        description: "Токен Adobe даёт доступ к API Creative Cloud/Experience Cloud вашей организации.",
        recommendation: "Отзовите токен в Adobe Developer Console и выпустите новый.",
        severity: Severity::High,
        confidence: Confidence::Medium,
        pattern: r"\b(p8e-[A-Za-z0-9_-]{32})\b",
        value_group: 1,
        min_entropy: 3.0,
        cwe: &["CWE-798"],
    },
    SecretRule {
        id: "VS-SEC-042",
        title: "Персональный токен Asana",
        description: "Токен Asana даёт доступ к задачам, проектам и рабочим областям вашего аккаунта.",
        recommendation: "Отзовите токен в настройках Asana (Apps → Developer) и выпустите новый.",
        severity: Severity::High,
        confidence: Confidence::Medium,
        pattern: r"\b(0/[0-9a-f]{32})\b",
        value_group: 1,
        min_entropy: 3.0,
        cwe: &["CWE-798"],
    },
    SecretRule {
        id: "VS-SEC-043",
        title: "Mailchimp API-ключ",
        description: "Ключ Mailchimp даёт доступ к спискам рассылки, подписчикам и кампаниям — это персональные данные и вектор для рассылок.",
        recommendation: "Отзовите ключ в настройках Mailchimp (Account → API keys) и выпустите новый.",
        severity: Severity::High,
        confidence: Confidence::High,
    ...
    SecretRule {
        id: "VS-SEC-044",
        title: "Токен SonarQube",
        description: "Токен SonarQube даёт доступ к результатам анализа кода, проектам и настройкам качества.",
        recommendation: "Отзовите токен в настройках SonarQube (Security → Tokens) и выпустите новый.",
        severity: Severity::High,
        confidence: Confidence::High,
        pattern: r"\b(sq[apu]_[0-9a-f]{40})\b",
        value_group: 1,
        min_entropy: 0.0,
        cwe: &["CWE-798"],
    },
    SecretRule {
        id: "VS-SEC-045",
        title: "Токен доступа Figma",
        description: "Токен Figma даёт доступ к файлам дизайна, проектам и командам вашего аккаунта.",
        recommendation: "Отзовите токен в настройках Figma (Personal access tokens) и выпустите новый.",
        severity: Severity::High,
        confidence: Confidence::High,
        pattern: r"\b(figd_[A-Za-z0-9_-]{40,})",
        value_group: 1,
        min_entropy: 0.0,
        cwe: &["CWE-798"],
    },
    SecretRule {
        id: "VS-SEC-046",
        title: "Auth-токен Sentry",
        description: "Auth-токен Sentry даёт доступ к проектам, событиям ошибок и настройкам организации через API.",
        recommendation: "Отзовите токен в настройках Sentry (Auth Tokens) и выпустите новый.",
        severity: Severity::High,
        confidence: Confidence::High,
        pattern: r"\b(sntrys_[A-Za-z0-9_=\-]{40,})",
        value_group: 1,
        min_entropy: 0.0,
        cwe: &["CWE-798"],
    },
    SecretRule {
        id: "VS-SEC-047",
        title: "Персональный токен Airtable",
        description: "Токен Airtable даёт доступ к базам, таблицам и записям, к которым он привязан.",
        recommendation: "Отзовите токен в настройках Airtable (Developer hub → Personal access tokens) и выпустите новый.",
        severity: Severity::High,
        confidence: Confidence::High,
        pattern: r"\b(pat[A-Za-z0-9]{14}\.[A-Za-z0-9]{64})\b",
        value_group: 1,
        min_entropy: 0.0,
        cwe: &["CWE-798"],
    },
    SecretRule {
        id: "VS-SEC-048",
        title: "Токен интеграции Notion (ntn_)",
        description: "Токен интеграции Notion в новом формате ntn_ даёт доступ к подключённым страницам и базам.",
        recommendation: "Отзовите токен в настройках интеграции Notion и выпустите новый.",
        severity: Severity::High,
        confidence: Confidence::High,
        pattern: r"\b(ntn_[A-Za-z0-9]{40,})",
        value_group: 1,
        min_entropy: 0.0,
        cwe: &["CWE-798"],
    },
    SecretRule {
        id: "VS-SEC-049",
        title: "Токен регистрации GitLab Runner",
        description: "Токен регистрации GitLab Runner позволяет подключить свой раннер к проекту и перехватывать задания CI вместе с их секретами.",
        recommendation: "Сбросьте токен в настройках CI/CD GitLab и выпустите новый.",
        severity: Severity::High,
        confidence: Confidence::High,
    ...
    SecretRule {
        ...
        description: "Токен триггера пайплайна GitLab позволяет запускать пайплайны проекта и, через них, выполнять код в CI.",
        recommendation: "Отзовите токен в настройках CI/CD → Pipeline triggers и выпустите новый.",
        severity: Severity::High,
        confidence: Confidence::High,
        pattern: r"\b(glptt-[0-9a-f]{40})\b",
        value_group: 1,
        min_entropy: 0.0,
        cwe: &["CWE-798"],
    },
    SecretRule {
        ...
        description: "PAT Docker Hub позволяет публиковать и скачивать образы от вашего имени — вектор атаки на цепочку поставок контейнеров.",
        recommendation: "Отзовите токен в настройках Docker Hub (Security) и выпустите новый.",
        severity: Severity::High,
        confidence: Confidence::High,
        pattern: r"\b(dckr_pat_[A-Za-z0-9_-]{27})\b",
        value_group: 1,
        min_entropy: 0.0,
        cwe: &["CWE-798"],
    },
    SecretRule {
        id: "VS-SEC-052",
        title: "Токен API RubyGems",
        description: "Токен RubyGems позволяет публиковать гемы от вашего имени. Утечка ведёт к компрометации цепочки поставок Ruby.",
        recommendation: "Отзовите токен на rubygems.org (Settings → API keys) и выпустите новый с ограниченной областью.",
        severity: Severity::Critical,
        confidence: Confidence::High,
        pattern: r"\b(rubygems_[0-9a-f]{48})\b",
        value_group: 1,
        min_entropy: 0.0,
        cwe: &["CWE-798"],
    },
    SecretRule {
        id: "VS-SEC-053",
        title: "Токен доступа Pulumi",
        description: "Токен Pulumi даёт доступ к состоянию инфраструктуры (state) и секретам стеков в Pulumi Cloud.",
        recommendation: "Отзовите токен в настройках Pulumi (Access Tokens) и выпустите новый.",
        severity: Severity::Critical,
        confidence: Confidence::High,
        pattern: r"\b(pul-[0-9a-f]{40})\b",
        value_group: 1,
        min_entropy: 0.0,
        cwe: &["CWE-798"],
    },
    SecretRule {
        ...
        description: "Account SID Twilio идентифицирует аккаунт и в паре с auth-токеном даёт полный доступ к API отправки SMS и звонков.",
        recommendation: "Не храните SID в коде рядом с секретом; держите учётные данные в переменных окружения.",
        severity: Severity::Medium,
        confidence: Confidence::Medium,
        pattern: r"\b(AC[0-9a-f]{32})\b",
        value_group: 1,
        min_entropy: 3.0,
        cwe: &["CWE-798"],
    },
    SecretRule {
        id: "VS-SEC-055",
        title: "Секрет подписи вебхука Stripe",
        description: "Секрет whsec_ проверяет подпись вебхуков Stripe. Зная его, атакующий подделывает события (например, «оплата прошла») и обманывает вашу бизнес-логику.",
        recommendation: "Смените секрет вебхука в дашборде Stripe и храните его в переменных окружения.",
        severity: Severity::High,
        confidence: Confidence::High,
        pattern: r"\b(whsec_[A-Za-z0-9]{32,})",
        value_group: 1,
        min_entropy: 0.0,
        cwe: &["CWE-798"],
    },
...
mod tests {
    ...
    #[test]
    fn finds_saas_provider_tokens() {
        assert!(ids("t = \"ATATT3xFfGF0kR9mZ2qW7xL4nP8vT1yB6cJ3wZ\"", "c.py").contains(&"VS-SEC-032"));
        assert!(ids("t = \"dp.pt.kR9mZ2qW7xL4nP8vT1yB6cJ3dH5gF0sAeUoIqWzXvB\"", "c.js").contains(&"VS-SEC-034"));
        assert!(ids("t = \"hvs.kR9mZ2qW7xL4nP8vT1yB6cJ3dH5g\"", "c.rb").contains(&"VS-SEC-036"));
        assert!(ids("k = \"9f8e7d6c5b4a3f2e1d0c9b8a7f6e5d4c-us5\"", "c.go").contains(&"VS-SEC-043"));
    }
