...
const SECTIONS: HelpSection[] = [
  ...
  },
  {
    ...
    items: [
      ...
      { icon: "terminal", term: "Внешние сканеры", desc: "Semgrep, Bandit, Trivy, gitleaks и другие подключаются по желанию; их находки объединяются с встроенными без дублей. Это отдельные консольные программы, поэтому VulnScope запускает их без окна — иначе при старте по экрану пробегала бы дюжина чёрных прямоугольников. Список того, что реально отработало, всегда виден в отчёте." },
  ...
  },
  {
    ...
    items: [
      ...
      { icon: "conversion_path", term: "Пути атаки", desc: "На дашборде — сами маршруты «точка входа → опасный вызов», отсортированные по опасности. Клик открывает находку, «Показать все пути» раскрывает список целиком." },
      { icon: "priority_high", term: "С чего начать", desc: "Пять файлов с наибольшим риском — ранжированы теми же весами, что и оценка. Клик открывает список находок этого файла." },
      { icon: "show_chart", term: "Динамика", desc: "Полоса на дашборде показывает, растёт или падает число находок за последние сканы; клик открывает отчёт с полным графиком." },
      { icon: "login", term: "Точка входа", desc: "Движок определяет, откуда входит ввод: HTTP-запрос, аргумент командной строки, переменная окружения, стандартный ввод или адрес страницы (DOM)." },
    ],
  },
  {
    icon: "account_tree",
  ...
  },
  {
    ...
    items: [
      { icon: "filter_alt", term: "Фильтры", desc: "По важности, файлу, поиску, «только новые» и «только достижимые». Активные фильтры показаны строкой, каждый снимается одним кликом; счётчики следуют за фильтром и показывают, что даст его нажатие." },
      { icon: "person", term: "Триаж по автору", desc: "«Автор строки» в карточке — кнопка: сужает список до всего, чего последним касался этот человек." },
