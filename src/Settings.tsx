        ...
        {shows("engines") && (
          <div className="set-grid">
            ...
            <Toggle
              ...
              hint={t("Совсем без сети: зависимости всё равно разбираются и считаются, но CVE не запрашиваются, проверка обновлений не выполняется. Наружу VulnScope отправляет только имена и версии пакетов в OSV.dev и запрос номера последней версии — код не уходит никогда.")}
              on={s.offline}
              onChange={(v) => apply({ offline: v })}
            />
            <Toggle
              label={t("Проверять обновления")}
              hint={t("Один раз за запуск спрашивает у GitHub номер последней версии и показывает подсказку, если вышла новее. Ничего не скачивает и не запускает — ссылку вы открываете сами. Офлайн-режим отключает и это.")}
              on={s.checkUpdates}
              onChange={(v) => apply({ checkUpdates: v })}
            />
            <Num
              label={t("Кэш OSV")}
              unit={t("дней")}
